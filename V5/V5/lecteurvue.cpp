...
void LecteurVue::lancerDiapo()
{
    ...
    rangImg.setNum(LeLecteur->imageCourante()->getRang());
    ui->nbr_image->setText(rangImg);
    ...
    while (true)
    {
        ...
        ui->nbr_image->setText(rangImg);
        ui->nbr_total_image->setText(nbrImage);
        rangImg.setNum(LeLecteur->imageCourante()->getRang());
        ui->nbr_image->setText(rangImg);
...
void LecteurVue::chargerLeDiapo()
{
    ...
    ui->lTitre->setText(imageTitre);
    ui->lImage->setPixmap(QPixmap(imageAffichee));    
    nbrImage.setNum(LeLecteur->nbImages());
    ui->nbr_total_image->setText(nbrImage);
    modeManuel();
    ui->nbr_image->setEnabled(true);
    ui->nbr_total_image->setEnabled(true);
    ui->slash->setEnabled(true);
...
void LecteurVue::viderLeDiapo()
{
    ...
    ui->bSuivant->setEnabled(false);
    ui->bLancerDiapo->setEnabled(false);
    ui->nbr_image->setVisible(true);
    ui->nbr_total_image->setVisible(true);
    ui->slash->setVisible(true);
    modeManuel();
...
void LecteurVue::aide()
{
    ...
    msgBox.setText("Version : 5 \n "
                   "Date de dernière modification : 05/06/2023 \n "
